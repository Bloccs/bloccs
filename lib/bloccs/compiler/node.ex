defmodule Bloccs.Compiler.Node do
  ...
  defp render_pipeline(p) do
    ...
    producer_opts =
      "[name: #{inspect(p.producer_name)}, buffer: #{inspect(p.buffer)}" <>
        if(p.delay, do: ", delay: #{p.delay}]", else: "]")
    ...
    rate_limiting_opt =
      case p.rate do
        ...
        %{allowed: allowed, interval_ms: interval} ->
          ",\n        rate_limiting: [allowed_messages: #{allowed}, interval: #{interval}]"
      end
    ...
    defmodule #{inspect(p.module)} do
      ...
      def start_link(opts \\\\ []) do
        Broadway.start_link(__MODULE__,
          ...
          producer: [
            module: {Bloccs.Producer, #{producer_opts}},
            concurrency: 1#{rate_limiting_opt}
          ],
